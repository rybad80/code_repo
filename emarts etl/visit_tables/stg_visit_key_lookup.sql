with xwalks as (
    select
        ...
        '' as patient_id,  /* Remove patient_id for Clarity due to patient merges */
        'clarity' as source_name
    from
        {{source('clarity_ods','pat_enc')}}
    union all
    select
        ctr::varchar(50) as encounter_id,
        patient_number::varchar(50) as patient_id,
        'fastrack' as source_name
    from
        {{source('fastrack_ods','pat_disc')}}
    union all
    select
        enc_id::varchar(50) as encounter_id,
        pat_id::varchar(50) as patient_id,
        'idx' as source_name
    from
        {{source('manual_ods', 'idx_visit')}}
    union all
    select
        enc_id::varchar(50) as encounter_id,
        pat_id::varchar(50) as patient_id,
        'idxrad' as source_name
    from
       {{source('manual_ods', 'idxrad_visit')}}
),
