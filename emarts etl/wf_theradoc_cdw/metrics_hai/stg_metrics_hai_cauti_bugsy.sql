...
where
    ...
    and bugsy_custom_infection_classes.eventtype = 'UTI'
    and bugsy_custom_infection_classes.lda_associated_yn = 'Y'
    and (
            bugsy_custom_infection_classes.urinarycath in ('IN PLACE')
            or (
            bugsy_custom_infection_classes.urinarycath in ('REMOVE')
            and date(bugsy_custom_infection_classes.removal_insertion_dttm) >= date(bugsy_custom_infection_classes.inf_date) - 1
        )
    )
