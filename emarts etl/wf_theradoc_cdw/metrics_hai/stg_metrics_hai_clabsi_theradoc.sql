...
infclass as (
    select
        infection_surveillance_class.inf_surv_key,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%BSI%' then 'CLABSI' end) as hai_type,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%BSI%' then 'BSI' end) as event_type,
...
from
    ...
    inner join infclass
        on infclass.inf_surv_key = surv.inf_surv_key
            and infclass.centralline = 'Y'
            and infclass.hai_type = 'CLABSI'
...
where (survvisit.seq_num is null or survvisit.seq_num = 1)
    ...
    and surv.inf_acq_type = 'HOSPITAL-ASSOCIATED' and upper(surv.assigned_to_icp) not like '%SIGN%'
