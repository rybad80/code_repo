...
infclass as (
    select
        infection_surveillance_class.inf_surv_key,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%UTI%' then 'CAUTI' end) as hai_type,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%UTI%' then 'UTI' end) as event_type,
...
from
    ...
    inner join infclass
        on infclass.inf_surv_key = surv.inf_surv_key
            and infclass.hai_type = 'CAUTI'
            and infclass.urinarycath in ('REMOVE', 'IN PLACE')
