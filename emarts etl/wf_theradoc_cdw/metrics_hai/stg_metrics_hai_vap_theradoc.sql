...
infclass as (
    select
        infection_surveillance_class.inf_surv_key,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%Pneumonia%' then 'VAP' end) as hai_type,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%Pneumonia%' then 'PNEU' end) as event_type,
...
from
    ...
    inner join infclass
        on infclass.inf_surv_key = surv.inf_surv_key
            and infclass.hai_type = 'VAP'
            and infclass.ventused = 'Y'
