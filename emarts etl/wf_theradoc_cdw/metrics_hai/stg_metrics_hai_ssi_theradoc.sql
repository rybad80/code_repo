...
surg as (
    ...
    from
        ...
        inner join {{source('cdw', 'or_log')}} as or_log
            on or_log.log_key = infection_surveillance_surgery.log_key
        inner join {{source('cdw', 'or_log_all_procedures')}} as or_log_all_procedures
            on or_log_all_procedures.log_key = or_log.log_key
                and or_log_all_procedures.seq_num = infection_surveillance_surgery.or_seq_num
                and or_log_all_procedures.or_proc_key = infection_surveillance_surgery.or_proc_key
        inner join {{source('cdw', 'or_procedure')}} as or_procedure
            on or_procedure.or_proc_key = or_log_all_procedures.or_proc_key
        inner join {{source('cdw', 'cdw_dictionary')}} as dict_nhsn
            on dict_nhsn.dict_key = or_procedure.dict_rpt_grp2_key
        inner join {{source('cdw', 'provider')}} as provider
            on provider.prov_key = or_log.room_prov_key
),

infclass as (
    select
        infection_surveillance_class.inf_surv_key,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%SSI (surgical site infection)%' then 'SSI' end) as hai_type,
        max(case when infection_surveillance_class.inf_surv_cls_nm like '%SSI (surgical site infection)%' then 'SSI' end) as event_type,
...
select distinct
    ...
    surg.full_nm::varchar(50) as or_location,
    ...
    null as port,
    null as postproc,
    surg.dict_abbr::varchar(5) as proccode,
    surg.or_proc_nm::varchar(36) as proccodedesc,
    to_date(surg.surg_dt_key, 'yyyymmdd') as procdate
