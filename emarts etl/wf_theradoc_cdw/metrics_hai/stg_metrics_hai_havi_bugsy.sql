...
from
    ...
    left join {{ref('metrics_hai_pathogen_lookup')}} as metrics_hai_pathogen_lookup
        on upper(infection_surveillance_micro.organism_nm) = upper(metrics_hai_pathogen_lookup.organism_nm)
...
where
    ...
    and metrics_hai_pathogen_lookup.pathogen in ('ADV', 'PFLU', 'RHNV', 'RSV', 'FLUA', 'FLUB', 'VIRUS', 'H1N1', 'RESPV', 'ASTTU', 'ENTRO', 'NORO', 'ROTA', 'CORTU', 'META', 'HMPV', 'SAPO', 'CORV', 'FLUC', 'FLU', 'NCOV2019')
    and bugsy_custom_infection_classes.infection_event = 'Healthcare-Associated Infection'
    and (
       (
            bugsy_custom_infection_classes.eventtype = 'EENT'
            and bugsy_custom_infection_classes.ssi_event_type = 'Upper Respiratory Tract'
        )
        or (
            bugsy_custom_infection_classes.eventtype = 'GI'
            and bugsy_custom_infection_classes.ssi_event_type = 'Gastroenteritis'
        )
        or (
            bugsy_custom_infection_classes.eventtype in ('PNEU')
            and bugsy_custom_infection_classes.lda_associated_yn = 'N'
        )
    )
