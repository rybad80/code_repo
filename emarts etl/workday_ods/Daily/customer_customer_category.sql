{{ config(
    ...
    unique_key = ['customer_wid', 'customer_category_wid'],
...
select distinct  -- As a bridge table, we need to find all distinct combinations of these fields
    ...
    customer_customer_data_customer_category_reference_wid as customer_category_wid,
    customer_customer_data_customer_category_reference_customer_category_id as customer_category_id,
    cast({{
    ...
    {%- if is_incremental() %}
        and md5 not in (
            ...
            where
                ...
                and customer_category_wid = get_customers.customer_customer_data_customer_category_reference_wid
        )
    {%- endif %}
