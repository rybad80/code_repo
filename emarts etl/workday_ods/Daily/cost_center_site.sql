{{
    config(
        ...
        merge_update_columns = ['cost_center_site_wid', 'cost_center_site_id', 'cost_center_site_name', 'cost_center_site_code', 'availibility_date', 'last_updated_date', 'inactive_ind', 'inactive_date', 'organization_type_wid', 'organization_type_id', 'organization_subtype_wid', 'organization_subtype_id', 'md5', 'upd_dt', 'upd_by']
    )
}}
select distinct
    ...
    organization_organization_data_name as cost_center_site_name,
    organization_organization_data_organization_code as cost_center_site_code,
...
where
    ...
    and organization_data_organization_subtype_reference_organization_subtype_id = 'Cost_Center_Site'
    and 1 = 1     
    {%- if is_incremental() %}
        and md5 not in (
            select md5
            from
                {{ this }}
            where
                cost_center_site_wid = get_organizations.organization_organization_reference_wid
        )
    {%- endif %}