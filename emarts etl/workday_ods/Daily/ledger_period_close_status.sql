...
with ledger_period_close_open_adj as (
    ...
    where
        lower(ledger_period_status_id) in ('closed','open','adjustments_only')
),
ledger_period_close_in_prog as (
    select distinct
        company_wid,
        company_reference_id as company_id,
        fiscal_period_wid,
        ...
        ledger_period as ledger_period_descriptor,
        book_code as book_code_descriptor,
        'multiple users' as last_updated_by,
        max(to_timestamp(substring(last_update_date,1,10) || ' ' || substring(last_update_date,12,8) , 'yyyy-mm-dd hh24:mi:ss')) as last_update_date,
        substring(last_update_date,21,5) as last_update_date_utc_offset
    from
        {{source('workday_ods', 'workday_ledger_period_close_status')}} as workday_ledger_period_close_status
    where
        lower(ledger_period_status_id) in ('close_in_progress')
    group by
        company_wid,
        company_id,
        fiscal_period_wid,
    ...
    {%- if is_incremental() %}
        and md5 not in (
            ...
            from
                {{ this }}
            where
                company_wid = ledger_period_union.company_wid
                and fiscal_period_wid = ledger_period_union.fiscal_period_wid
        )
    {%- endif %}