...
select
    ...
    'WORKDAY' as upd_by
from
    {{source('workday_ods', 'workday_journal_line_eliminations')}} as workday_journal_line_eliminations
where
    1 = 1
    {%- if is_incremental() %}
        and md5 not in (
            ...
            from
                {{ this }}
            where
                journal_wid = workday_journal_line_eliminations.journal_wid
                and journal_line_wid = workday_journal_line_eliminations.journal_line_wid
        )
    {%- endif %}