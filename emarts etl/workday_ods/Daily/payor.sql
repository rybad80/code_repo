{{
    config(
        ...
        merge_update_columns = ['payor_wid', 'payor_id', 'payor_name', 'payor_code', 'availibility_date', 'last_updated_date', 'inactive_ind', 'inactive_date', 'organization_type_wid', 'organization_type_id', 'organization_subtype_wid', 'organization_subtype_id', 'md5', 'upd_dt', 'upd_by']
    )
}}
select distinct
    ...
    organization_organization_data_name as payor_name,
    organization_organization_data_organization_code as payor_code,
...
where
    organization_data_organization_type_reference_organization_type_id = 'ORGANIZATION_TYPE-6-42'
    ...
    {%- if is_incremental() %}
        and md5 not in (
            ...
            from
                {{ this }}
            where
                payor_wid = get_organizations.organization_organization_reference_wid
        )
    {%- endif %}