...
AND CDWPRD.ADMIN.WORKDAY_LEDGER_ACCOUNT.LEDGER_ACCT_ID IN (40000,50000,52500,53000,53050,54000)
AND POST_MONTH between add_months(date_trunc('month',current_date),-24) and add_months(last_day(current_date),-1)
GROUP BY 1,2,3
)
,
STEP2 AS (
...
LOC_NM,
AMT,
TRUNC(AVG(AMT) OVER(ORDER BY LOC_ID,POST_MONTH ROWS BETWEEN 4 PRECEDING AND 2 PRECEDING), 2) AS AVG_AMT
FROM STEP1
...
FROM STEP2
WHERE POST_MONTH between add_months(date_trunc('month',current_date),-15) and add_months(last_day(current_date),-1)
;
