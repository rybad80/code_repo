ALTER PROCEDURE "dbo"."sp_CHOP_LoadMortCases" as
...
SET NOCOUNT ON
			BEGIN
				...
				BEGIN TRAN
					INSERT INTO dbo.MortCases (CaseNumber, HospitalID, Mt30Stat, Mt30StatMeth,MortCase, MtOpD,  CreateDate, UpdateDate, UpdateBy, Mt365Stat, Mt365StatMeth)
					SELECT STG.CaseNumber,  4, STG.Mt30Stat, STG.Mt30StatMeth, STG.MortCase, STG.MtOpD,  getdate(), getdate(),'CHOP_AUTOMATION', STG.Mt365Stat, STG.Mt365StatMeth
			         FROM chop_sts_mortcases STG 
					 	  left join cases on stg.casenumber = cases.CaseNumber
					      LEFT JOIN dbo.mortcases TGT ON STG.CaseNumber = TGT.CaseNumber					    
					WHERE tgt.casenumber is null 
