...
SELECT  FR.VSI_KEY
	   ...
	   ,MIN(CASE WHEN F.FS_ID=40010893 AND MEAS_VAL = '2.5' THEN 453 
	             WHEN F.FS_ID=40010893 AND MEAS_VAL = '3' THEN 454
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '3.5' THEN 455
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '4' THEN 456
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '4.5' THEN 457
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '5' THEN 458
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '5.5' THEN 459
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '6' THEN 460
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '6.5' THEN 461
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '7' THEN 462
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '7.5' THEN 463
				 WHEN F.FS_ID=40010893 AND MEAS_VAL = '8' THEN 464
	   ...
	   ,3657  AS ENDOBRONCH_ISO_METH --N/A
...
group BY FR.VSI_KEY, PLACE_DT, REMOVE_DT, ENDOBRONCH_ISO
 ...
 SELECT 
	...
	,cast((CASE WHEN AIRWAYTYPE = 475 THEN ENDOBRONCH_ISO_METH ELSE NULL END) as integer) AIRWAYSIZELMA
	,cast((CASE WHEN AIRWAYTYPE = 476 THEN ENDOTRACH_SZ ELSE NULL END) as integer) AIRWAYSIZEINTUB
	,cast((CASE WHEN AIRWAYTYPE = 476 THEN CUFFED ELSE NULL END) as integer) CUFFED
	,cast((CASE WHEN AIRWAYTYPE in (476,477) THEN AIRWAYSITE ELSE NULL END) as integer) AIRWAYSITE
	,cast(ISNULL(ENDOBRONCH_ISO,2) as integer) EndobroncIso
	,cast((CASE WHEN ENDOBRONCH_ISO = 1 THEN ENDOBRONCH_ISO_METH ELSE NULL END) as integer) EndobroncIsoMeth
	,ANESREADY_TM EndOfInductDT
	,cast(2 as integer) as ICUTYPEVENT
 FROM
 (	   SELECT
             ENCS.*,
		     AIRWAY.*,
	   ...
	   FROM ENCS
		     LEFT JOIN (SELECT AIRWAY.*, 1 AIRWAY_SITU, ROW_NUMBER() OVER (PARTITION BY AIRWAY.VSI_KEY ORDER BY AIRWAY_PLACE_DT DESC, AIRWAY_REM_DT)  ROWNO
		     ...
		     LEFT JOIN (SELECT AIRWAY.*, ROW_NUMBER() OVER (PARTITION BY AIRWAY.VSI_KEY ORDER BY AIRWAY_PLACE_DT, AIRWAY_REM_DT DESC) ROWNO
		                 FROM AIRWAY JOIN ENCS ON ENCS.ANES_VSI_KEY = AIRWAY.VSI_KEY  
