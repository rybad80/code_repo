...
(SELECT  FLOW_REC.vsi_key
		...
		,ROW_NUMBER() OVER (PARTITION BY FLOW_REC.VSI_KEY ORDER BY FLOW_MEA.ENTRY_DT, FLOW_MEA.REC_DT) ICU_FIO2_ORDER 
...
(SELECT  FLOW_REC.vsi_key
		...
		-- ,ENCS.HANDOFF_TM
		 ...
		 ,ROW_NUMBER() OVER (PARTITION BY FLOW_REC.VSI_KEY ORDER BY FLOW_MEA.ENTRY_DT, FLOW_MEA.REC_DT) ICU_SPO2_ORDER
FROM  CDW.VISIT_ADDL_INFO VAI 
    JOIN CDW.FLOWSHEET_RECORD FLOW_REC ON FLOW_REC.VSI_KEY=VAI.VSI_KEY 
    INNER JOIN CDW.FLOWSHEET_MEASURE FLOW_MEA ON FLOW_MEA.FS_REC_KEY = FLOW_REC.FS_REC_KEY
    INNER JOIN CDW.FLOWSHEET FLOW ON FLOW.FS_KEY=FLOW_MEA.FS_KEY 
...
,ICU_TMP AS 
	( select  
		 ...
		 ROW_NUMBER() OVER (PARTITION BY VAI.VSI_KEY ORDER BY ENTRY_DT, REC_DT) ICU_TMP_ORDER  
		FROM 
...
, ICU_LABS AS
 (select RSLT.PROC_ORD_KEY
       ,RSLT.VISIT_KEY
	   , 1 AS ICU_LABS
	   ,MIN(CASE WHEN RSLT.RSLT_COMP_KEY = 50085 THEN RSLT_NUM_VAL END) AS PH
	   ,MIN(CASE WHEN RSLT.RSLT_COMP_KEY = 50692 THEN RSLT_NUM_VAL END) AS PCO2
	   ,MIN(CASE WHEN RSLT.RSLT_COMP_KEY = 51936 THEN RSLT_NUM_VAL END) AS PO2
	   ,MIN(CASE WHEN RSLT.RSLT_COMP_KEY = 51216 THEN RSLT_NUM_VAL END) AS BASEEXCESS
	   ,MIN(CASE WHEN RSLT.RSLT_COMP_KEY = 53433 THEN RSLT_NUM_VAL END) AS LACTATE
	   ,RSLT_DT --SELECT *
...
( 
		SELECT ENCS.*,
			   ...
			   ICU_HEMATOCRIT.*
		FROM ENCS 
				 LEFT JOIN ICU_FIO2 ON ICU_FIO2.VSI_KEY = ENCS.HSP_VAI_KEY AND ICU_FIO2_ORDER = 1
				 LEFT JOIN ICU_SPO2 ON ICU_SPO2.VSI_KEY = ENCS.HSP_VAI_KEY AND ICU_SPO2_ORDER = 1
				 LEFT JOIN ICU_TMP ON ICU_TMP.VSI_KEY = ENCS.HSP_VAI_KEY AND ICU_TMP_ORDER = 1
				 LEFT JOIN ICU_TMPSITE ON ICU_TMPSITE.VSI_KEY = ENCS.HSP_VAI_KEY AND TMP_SITE_ORDER = 1
