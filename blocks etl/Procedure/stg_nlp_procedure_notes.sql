...
with today_notes as (
    ...
    where
        line = 1
        and date(upd_dt) > current_date - 14
    union
    select
        order_proc_id
    from
...
proc_ids as (
    ...
    where
        ...
        and date(order_proc.upd_dt) > current_date - 14
),
lines as (
    select distinct
        proc_ids.pat_mrn_id,
...
select
    ...
    'PROC_ORDER_ID: ' || lines.order_proc_id || '\n'
        ...
        || listagg(lines.note, ' ') within group(order by lines.seq_num asc) as full_note --noqa: L012, PRS
from
    lines
