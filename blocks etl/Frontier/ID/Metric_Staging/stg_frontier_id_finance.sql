select distinct
    ...
    frontier_id_encounter_cohort.department_name,
...
from
    ...
    left join {{source('cdw_analytics', 'fact_financial_statistic')}} as fact_financial_statistic
        on frontier_id_encounter_cohort.hsp_acct_key = fact_financial_statistic.fs_acct_key
    ...
    left join {{source('cdw', 'hospital_account')}} as hospital_account
        on frontier_id_encounter_cohort.hsp_acct_key = hospital_account.hsp_acct_key
where
    --'admissions', 'discharges', 'observation patient day equivalents', 'ip patient days'
    --master_statistic.stat_cd in (8, 12, 14, 32)
    (master_statistic.stat_cd in (8, 12)        --'admissions', 'discharges'
        and frontier_id_encounter_cohort.id_ip_consult_ind = '1')
    or (master_statistic.stat_cd in (14, 32)    --'observation patient day equivalents', 'ip patient days'
        and frontier_id_encounter_cohort.id_ip_consult_ind = '1'
