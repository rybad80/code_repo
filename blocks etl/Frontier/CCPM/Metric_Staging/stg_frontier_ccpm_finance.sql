select distinct
    ...
    frontier_ccpm_encounter_cohort.patient_sub_cohort,
    frontier_ccpm_encounter_cohort.admission_department,
...
from
    ...
    left join {{source('cdw_analytics', 'fact_financial_statistic')}} as fact_financial_statistic
        on frontier_ccpm_encounter_cohort.hsp_acct_key = fact_financial_statistic.fs_acct_key
    ...
    left join {{source('cdw', 'hospital_account')}} as hospital_account
        on frontier_ccpm_encounter_cohort.hsp_acct_key = hospital_account.hsp_acct_key
where
