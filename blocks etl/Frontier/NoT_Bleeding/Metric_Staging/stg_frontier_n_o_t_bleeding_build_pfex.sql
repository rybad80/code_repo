...
select
    frontier_n_o_t_bleeding_encounter_cohort.visit_key,
    case
    ...
    'up' as direction,
    case
        when lower(question_id) in ('a1') then 'fp_n_o_t_bleeding_ease'
        when lower(question_id) in ('b9') then 'fp_n_o_t_bleeding_handling_ins'
        when lower(question_id) in ('o15') then 'fp_n_o_t_bleeding_staff'
        when lower(question_id) in ('o4') then 'fp_n_o_t_bleeding_ltr'
        when lower(question_id) in ('v60') then 'fp_n_o_t_bleeding_wait'
        when lower(section_name ) = 'care provider' then 'fp_n_o_t_bleeding_overall'
    else 'CHECK' end as metric_id,
    pfex_all.tbs_ind as num
from
    {{ ref('frontier_n_o_t_bleeding_encounter_cohort') }} as frontier_n_o_t_bleeding_encounter_cohort
    inner join {{ ref('pfex_all') }} as pfex_all
        on frontier_n_o_t_bleeding_encounter_cohort.visit_key = pfex_all.visit_key
