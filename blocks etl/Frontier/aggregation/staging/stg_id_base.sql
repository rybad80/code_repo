...
agg_base as (--region
    select
        ...
        num as pat_key
    from
        {{ ref('stg_frontier_id_build_g_ip_patient') }}
    union all
    select
        primary_key as visit_key,
        ...
        num as pat_key
    from
        {{ ref('stg_frontier_id_build_g_op_patient') }}
    group by
        primary_key,
        num
    --end region
...
select
    'Immune Dysregulation' as program_name,
    sub_cohort,
    frontier_id_encounter_cohort.mrn,
    frontier_id_encounter_cohort.pat_key,
    agg_base.visit_key,
    'Geo' as metric_name,
    case
        when frontier_id_encounter_cohort.fiscal_year
                    = year(add_months(current_date - 1, 6))
        then 'FYTD'
        when frontier_id_encounter_cohort.fiscal_year
                    = year(add_months(current_date - 1, 6)) - 1
                and frontier_id_encounter_cohort.encounter_date
                    < date(add_months(current_date, - 12))
        then 'PFYTD'
        when frontier_id_encounter_cohort.encounter_date
                    < date(current_date)
    ...
    1 as num
from agg_base
    inner join {{ ref('frontier_id_encounter_cohort')}} as frontier_id_encounter_cohort
        on agg_base.visit_key = frontier_id_encounter_cohort.visit_key
    inner join {{ ref('stg_patient') }} as stg_patient
        on frontier_id_encounter_cohort.pat_key = stg_patient.pat_key
where
