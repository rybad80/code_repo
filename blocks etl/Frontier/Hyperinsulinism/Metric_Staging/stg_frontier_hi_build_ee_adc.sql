...
select
    ...
    sum(stg_frontier_hi_finance.statistic_measure) as num,
    (cast(days_in_month.day_of_mm as decimal(6, 4))
    / count(distinct primary_key) over (partition by date_trunc('month', stg_frontier_hi_finance.post_date)))
    as average_daily_census_denom
from
    {{ ref('stg_frontier_hi_finance')}} as stg_frontier_hi_finance
    inner join days_in_month
        on stg_frontier_hi_finance.post_date_month_year = days_in_month.visual_month
where
