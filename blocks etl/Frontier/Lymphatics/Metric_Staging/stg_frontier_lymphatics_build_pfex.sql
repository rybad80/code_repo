select
    frontier_lymphatics_encounter_cohort.visit_key,
    case
    ...
    'up' as direction,
    case
        when lower(question_id) in ('a1') then 'fp_lymph_ease'
        when lower(question_id) in ('b9') then 'fp_lymph_handling_ins'
        when lower(question_id) in ('o15') then 'fp_lymph_staff'
        when lower(question_id) in ('o4') then 'fp_lymph_ltr'
        when lower(question_id) in ('v60') then 'fp_lymph_wait'
        when lower(section_name ) = 'care provider' then 'fp_lymph_overall'
...
from
    ...
    inner join {{ ref('pfex_all') }} as pfex_all
        on frontier_lymphatics_encounter_cohort.visit_key = pfex_all.visit_key
