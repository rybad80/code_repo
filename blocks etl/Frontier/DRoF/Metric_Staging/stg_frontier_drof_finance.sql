select
    ...
    frontier_drof_encounter_cohort.sub_cohort,
    frontier_drof_encounter_cohort.department_name,
...
from {{ ref('frontier_drof_encounter_cohort')}} as frontier_drof_encounter_cohort
    left join {{source('cdw_analytics', 'fact_financial_statistic')}} as fact_financial_statistic
        on frontier_drof_encounter_cohort.hsp_acct_key = fact_financial_statistic.fs_acct_key
    ...
    left join {{source('cdw', 'hospital_account')}} as hospital_account
        on frontier_drof_encounter_cohort.hsp_acct_key = hospital_account.hsp_acct_key
where
    -- 'admissions', 'discharges', 'observation patient day equivalents', 'ip patient days'
    master_statistic.stat_cd in (8, 12, 14, 32)
    and frontier_drof_encounter_cohort.drof_sub_cohort_ind = 1
