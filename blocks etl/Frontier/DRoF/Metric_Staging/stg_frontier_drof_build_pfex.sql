-- pfex:patient family experience
select
    frontier_drof_encounter_cohort.visit_key,
    case
    ...
    'up' as direction,
    case
        when lower(question_id) in ('a1') then 'fp_drof_ease'
        when lower(question_id) in ('b9') then 'fp_drof_handling_ins'
        when lower(question_id) in ('o15') then 'fp_drof_staff'
        when lower(question_id) in ('o4') then 'fp_drof_ltr'
        when lower(question_id) in ('v60') then 'fp_drof_wait'
        when lower(section_name ) = 'care provider' then 'fp_drof_overall'
    else 'CHECK' end as metric_id,
    pfex_all.tbs_ind as num
from
    {{ ref('frontier_drof_encounter_cohort') }} as frontier_drof_encounter_cohort
inner join {{ ref('pfex_all') }} as pfex_all
    on frontier_drof_encounter_cohort.visit_key = pfex_all.visit_key
where frontier_drof_encounter_cohort.drof_sub_cohort_ind = 1
    and frontier_drof_encounter_cohort.inpatient_ind = 1
