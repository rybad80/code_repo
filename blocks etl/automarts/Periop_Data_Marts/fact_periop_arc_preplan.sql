...
from {{ ref('fact_periop') }} as fact_periop
    inner join {{ source('cdw', 'or_case') }} as or_case
        on or_case.or_case_id = fact_periop.log_id
    inner join
...
where lower(smart_data_element_info.src_sys_val) = 'smartform 283' -- unique ID of the anes pre plan smart form
