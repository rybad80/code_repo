with numerators as (
    select
        date_trunc('month', stg_pcoti_report_emergent_transfers.icu_enter_date) as event_year_month,
        stg_pcoti_report_emergent_transfers.from_campus_name as campus_name,
        stg_pcoti_report_emergent_transfers.from_department_name as department_group_name,
        sum(1) as numerator_emergent_transfers
    from
        {{ ref('stg_pcoti_report_emergent_transfers') }} as stg_pcoti_report_emergent_transfers
    group by
        date_trunc('month', stg_pcoti_report_emergent_transfers.icu_enter_date),
        stg_pcoti_report_emergent_transfers.from_campus_name,
        stg_pcoti_report_emergent_transfers.from_department_name
),
...
select
    ...
    coalesce(numerators.numerator_emergent_transfers, 0) as numerator_emergent_transfers,
