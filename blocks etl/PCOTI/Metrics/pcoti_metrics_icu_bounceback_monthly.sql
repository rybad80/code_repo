with numerators as (
    select
        date_trunc('month', stg_pcoti_report_icu_bouncebacks.bb_icu_enter_date) as event_year_month,
        stg_pcoti_report_icu_bouncebacks.bb_from_campus_name as campus_name,
        stg_pcoti_report_icu_bouncebacks.bb_from_department_name as department_group_name,
        sum(1) as numerator_bouncebacks
    from
        {{ ref('stg_pcoti_report_icu_bouncebacks') }} as stg_pcoti_report_icu_bouncebacks
    group by
        date_trunc('month', stg_pcoti_report_icu_bouncebacks.bb_icu_enter_date),
        stg_pcoti_report_icu_bouncebacks.bb_from_campus_name,
        stg_pcoti_report_icu_bouncebacks.bb_from_department_name
),
...
select
    ...
    coalesce(numerators.numerator_bouncebacks, 0) as numerator_bouncebacks,
