...
provider_room_utilization as (
    select
        scc_qgenda_schedule_entries.schedulekey,
        scc_qgenda_schedule_entries.provider_id,
        ...
        case
            ...
            then 1 else 0 end as room_used
    from {{ref('scc_qgenda_schedule_entries')}} as scc_qgenda_schedule_entries
    left join visits_by_session on visits_by_session.am_or_pm = scc_qgenda_schedule_entries.session_type
        ...
        and visits_by_session.ext_id = scc_qgenda_schedule_entries.provider_id
    where (lower(room_type) like '%overfill%'
        or exam_room_ind = 1) -- provider-level utilization metrics include exam rooms and overfill rooms
    and business_day_ind = 1
    and scc_qgenda_schedule_entries.provider_id is not null
...
select * from provider_room_utilization
