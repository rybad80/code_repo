select
    frontier_ctis_encounter_cohort.visit_key,
    case
    ...
    'up' as direction,
    case
        when lower(question_id) in ('a1') then 'frontier_ctis_ease'
        when lower(question_id) in ('b9') then 'frontier_ctis_handling_ins'
        when lower(question_id) in ('o15') then 'frontier_ctis_staff'
        when lower(question_id) in ('o4') then 'frontier_ctis_ltr'
        when lower(question_id) in ('v60') then 'frontier_ctis_wait'
        when lower(section_name ) = 'care provider' then 'frontier_ctis_overall'
...
from
    ...
    inner join {{ ref('pfex_all')}} as pfex_all
        on frontier_ctis_encounter_cohort.visit_key = pfex_all.visit_key
