select
    ...
    frontier_hi_encounter_cohort.department_name,
...
from {{ ref('frontier_hi_encounter_cohort')}} as frontier_hi_encounter_cohort
    left join {{source('cdw_analytics', 'fact_financial_statistic')}} as fact_financial_statistic
        on frontier_hi_encounter_cohort.hsp_acct_key = fact_financial_statistic.fs_acct_key
    ...
    left join {{source('cdw', 'hospital_account')}} as hospital_account
        on frontier_hi_encounter_cohort.hsp_acct_key = hospital_account.hsp_acct_key
