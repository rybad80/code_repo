select
    frontier_hi_encounter_cohort.visit_key,
    case
    ...
    'up' as direction,
    case
        when lower(question_id) in ('a1') then 'frontier_hi_ease'
        when lower(question_id) in ('b9') then 'frontier_hi_handling_ins'
        when lower(question_id) in ('o15') then 'frontier_hi_staff'
        when lower(question_id) in ('o4') then 'frontier_hi_ltr'
        when lower(question_id) in ('v60') then 'frontier_hi_wait'
        when lower(section_name ) = 'care provider' then 'frontier_hi_overall'
...
from
    ...
    inner join {{ ref('pfex_all')}} as pfex_all
        on frontier_hi_encounter_cohort.visit_key = pfex_all.visit_key
