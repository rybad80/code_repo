...
select
    'CCPM: Average Daily Census' as metric_name,
    {{
    ...
    stg_frontier_ccpm_finance.post_date as metric_date,
    'sum' as num_calculation,
    'rate' as metric_type,
    'up' as direction,
    'fp_ccpm_avg_dc' as metric_id,
    sum(stg_frontier_ccpm_finance.statistic_measure) as num,
    (cast(days_in_month.day_of_mm as decimal(6, 4))
    / count(distinct primary_key) over (partition by date_trunc('month', stg_frontier_ccpm_finance.post_date)))
    as average_daily_census_denom
from
    {{ ref('stg_frontier_ccpm_finance') }} as stg_frontier_ccpm_finance
    inner join days_in_month
        on stg_frontier_ccpm_finance.post_date_month_year = days_in_month.visual_month
