...
,PAT_CURR_WT AS 
 (SELECT CATH_STUDY.REFNO,
	     ...
	     ROW_NUMBER() OVER (PARTITION BY CATH_STUDY.REFNO ORDER BY FLOW_MEA.REC_DT DESC) REC_ORDER
FROM 
    CATH_STUDY
	JOIN $$CDW..VISIT_ADDL_INFO VAI ON VAI.PAT_KEY = CATH_STUDY.PAT_KEY 
	JOIN $$CDW..FLOWSHEET_RECORD FLOW_REC ON FLOW_REC.VSI_KEY=VAI.VSI_KEY 
    JOIN $$CDW..FLOWSHEET_MEASURE FLOW_MEA ON FLOW_MEA.FS_REC_KEY = FLOW_REC.FS_REC_KEY AND CAST(FLOW_MEA.REC_DT AS DATE) <= CATH_STUDY.STUDATE
    JOIN $$CDW..FLOWSHEET FLOW ON FLOW.FS_KEY=FLOW_MEA.FS_KEY
WHERE FS_ID IN (14)
)
...
--SELECT *
 from CATH_STUDY JOIN $$CDW..PROCEDURE_ORDER_RESULT RSLT ON RSLT.PAT_KEY = CATH_STUDY.PAT_KEY AND CAST(RSLT_DT AS DATE) <= STUDATE
                 join $$CDW..RESULT_COMPONENT COMP ON RSLT.RSLT_COMP_KEY = COMP.RSLT_COMP_KEY
...
( SELECT CATH_STUDY.REFNO
		 ...
		 ,PAT_MRN_ID
        ,ROW_NUMBER() OVER (PARTITION BY CATH_STUDY.REFNO ORDER BY EXTRACT(EPOCH FROM SH_ACC_TM-AUTOTIME) DESC ,SEQNO ) REC_ORDER
...
, PREPROC_O2SAT AS
 (SELECT
		 ...
		 EXTRACT(EPOCH FROM SH_ACC_TM-FLOW_MEA.REC_DT) diff,
	     ROW_NUMBER() OVER (PARTITION BY REFNO ORDER BY EXTRACT(EPOCH FROM SH_ACC_TM-FLOW_MEA.REC_DT)) REC_ORDER
FROM 
    CATH_STUDY
	JOIN $$CDW..VISIT_ADDL_INFO VAI ON VAI.PAT_KEY = CATH_STUDY.PAT_KEY 
	JOIN $$CDW..FLOWSHEET_RECORD FLOW_REC ON FLOW_REC.VSI_KEY=VAI.VSI_KEY 
    ...
    JOIN $$CDW..FLOWSHEET FLOW ON FLOW.FS_KEY=FLOW_MEA.FS_KEY 
WHERE FS_ID IN (10)
   AND EXTRACT(EPOCH FROM SH_ACC_TM-FLOW_MEA.REC_DT) BETWEEN 0 and 2592000
)
...
--SELECT * FROM SENSIS_DICIP5075
 FROM CATH_STUDY CATH JOIN SENSIS_IMARWY IMARWY  ON CATH.REFNO = IMARWY.REFNO
                          LEFT JOIN SENSIS_DICIP5075 IP5075 ON IMARWY.IP5075 = IP5075.CODE
...
FROM CATH_STUDY CATH LEFT JOIN SENSIS_APTVER2 APTVER2 ON CATH.REFNO = APTVER2.REFNO
                     ...
                     LEFT JOIN (SELECT *, ROW_NUMBER() OVER (PARTITION BY REFNO ORDER BY SEQNO) ROW FROM SENSIS_PHYS )PHYS ON PHYS.REFNO = CATH.REFNO AND ROW = 1
...
( 
  SELECT  
         PN.REFNO
		,min(DICPNNAME.CODE) RECORDER 
   FROM 
        SENSIS_PN PN 
		LEFT JOIN SENSIS_DICPNNAME DICPNNAME ON PN.PNNAME = DICPNNAME.CODE
  WHERE
        PN.STAFF = 6
 GROUP BY
        PN.REFNO
...
SELECT   DISTINCT 
      ...
      ,COALESCE(CAST(CONTRASTVOL AS INTEGER),0) CONTRASTVOL
