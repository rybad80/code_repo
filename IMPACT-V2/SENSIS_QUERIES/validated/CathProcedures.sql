...
WHERE POCT.PRTYPE <> 10 --Fluoro
    ...
    SELECT
        SURG_ENC_ID
		,PEDPR.IP5010 SPECIFICPROCID
		,ROW_NUMBER() OVER (PARTITION BY PAT_MRN_ID, STUDATE ORDER BY SEQNO) SORT
  FROM CATH_STUDY STUDY JOIN SENSIS_PEDPR PEDPR ON STUDY.REFNO = PEDPR.REFNO
  where IP5010 is not null